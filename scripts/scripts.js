...
tasksContainer.addEventListener('click', (e) => {
	if (e.target.tagName.toLowerCase() === 'input') {
		...
		const selectedTask = activeList.tasks.find(
...
function renderTaskCount(activeList) {
	const incompleteTaskCount = activeList.tasks.filter(
		(task) => !task.complete
	).length;
	...
	if (activeList.tasks.length === 0) {
		...
		clearCompletedTasksButton.style.display = 'none';
	} else {
		remainingTasks.innerText = taskCountString;
		clearCompletedTasksButton.style.display = '';
	}
}
...
function renderLists() {
	savedLists.forEach((list) => {
		...
		if (list.id === activeListId) {
			newListElement.classList.toggle('active-list');
		}
